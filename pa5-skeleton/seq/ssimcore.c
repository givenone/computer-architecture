...
static void update_state()
{
    ...
    if (mem_write) {
      ...
      if(mem_byte)
      {
          byte_t temp = mem_data;
          set_byte_val(mem,mem_addr,temp);
      }
    else      set_word_val(mem, mem_addr, mem_data);
...
static byte_t sim_step()
{
    ...
    if (gen_mem_read()) {
      if(mem_byte){
          byte_t temp;
          dmem_error = dmem_error || !get_byte_val(mem,mem_addr,&temp);
          valm = 0xff & temp;
      }
      else dmem_error = dmem_error || !get_word_val(mem, mem_addr, &valm);
      if (dmem_error) {
