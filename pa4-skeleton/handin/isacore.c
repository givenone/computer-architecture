...
cc_t compute_cc(alu_t op, word_t argA, word_t argB)
{
	...
	break;
    case A_MUL:
		ovf = (((word_t) argB < 0) && ((word_t) argA == 0x8000000000000000)) || ((argB != 0) && (((((word_t) val) / ((word_t) argB))) != ((word_t) argA)) ) ;
    break;
    case A_DIV:
    ovf = ((word_t) argA==0) ? TRUE : FALSE;
    break;
    default:
	ovf = FALSE;
    }
...
//	byte_t lsb = val & 0xff;
	bool_t temp;
	//if(M_BYTE) temp = set_byte_val(s->m, cval, lsb);
	//else
	    temp = set_word_val(s->m, cval, val);
	if (!temp) {
	    if (error_file)
		fprintf(error_file,
	...
	//if(M_BYTE) {
	  //  byte_t jinsook = 0;
	   // jinsoo = get_byte_val(s->m, cval, &jinsook);
	    //val = 0x0ff & jinsook;
	//}
	//else
	    jinsoo = get_word_val(s->m, cval, &val);
	if (!jinsoo)
	    return STAT_ADR;
